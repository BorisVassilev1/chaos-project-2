...
Transformation::Transformation(const glm::vec3 &position) : Transformation(position, glm::vec3(0), glm::vec3(1)) {}

Transformation::Transformation(const glm::vec3 &position, const glm::vec3 &rotation, const glm::vec3 &scale)
	: position(position), rotation(rotation), scale(scale) {
	updateWorldMatrix();
}
Transformation::Transformation(glm::mat4 mat) : worldMatrix(mat) { updateVectors(); }

glm::mat4 &Transformation::getWorldMatrix() { return worldMatrix; }
...
void Transformation::updateVectors() { decomposeTransform(worldMatrix, position, rotation, scale); }

// stolen from Hazel engine:
// https://github.com/TheCherno/Hazel/blob/master/Hazel/src/Hazel/Math/Math.cpp
bool Transformation::decomposeTransform(const glm::mat4 &transform, glm::vec3 &translation, glm::vec3 &rotation,
										glm::vec3 &scale) {
...
bool Transformation::operator==(const Transformation &other) { return this->worldMatrix == other.worldMatrix; }


glm::vec3 Transformation::forward() { return (getWorldMatrix() * glm::vec4(0, 0, 1, 0)).xyz(); }
